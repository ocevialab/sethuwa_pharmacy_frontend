                        ...
                        {filteredOptions.length > 0 ? (
                            filteredOptions.map((option) => (
                                <li
                                    ...
                                    className={`${option?.value === localSelectedOption?.value ? 'active' : ''} ${option?.disabled ? 'opacity-50 text-muted' : ''}`}
                                    style={option?.disabled ? { pointerEvents: 'none' as const } : undefined}
                                    title={option?.disabled ? (option?.disabledReason as string) || 'Not available' : undefined}
