...
const MedicineRestore: React.FC = () => {
    const [medicines, setMedicines] = useState<Medicine[]>([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const navigate = useNavigate();

    // Fetch deleted medicines
    const fetchDeletedMedicines = useCallback(async () => {
        try {
            setLoading(true);
            setError(null);
            const deletedItems = await medicineService.getDeletedMedicines();
            setMedicines(deletedItems);
        } catch (err) {
            setError(err instanceof Error ? err.message : 'Failed to load deleted medicines');
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: err instanceof Error ? err.message : 'Failed to load deleted medicines',
            });
        } finally {
            setLoading(false);
        }
    ...
    useEffect(() => {
        fetchDeletedMedicines();
    }, [fetchDeletedMedicines]);

    // Handle restore
    const handleRestore = async (medicineId: string, name: string) => {
        ...
        if (result.isConfirmed) {
            try {
                await medicineService.restoreMedicine(medicineId);
                Swal.fire({
                    icon: 'success',
                    title: 'Restored!',
                    text: 'Medicine has been restored.',
                    timer: 2000,
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: err instanceof Error ? err.message : 'Failed to restore medicine',
                });
            }
        }
    };
    ...
    const columns = [
        ...
        },
        {
            ...
            header: () => 'Category',
            cell: (info: any) => <span className="badge bg-info">{info.getValue()}</span>
        },
        {
        ...
        },
        {
            ...
            header: () => 'Product SKU',
            cell: (info: any) => <span className="badge bg-primary">{info.getValue() || 'N/A'}</span>
        },
        {
                ...
                const actions = [
                    {
                        label: "Restore",
                        icon: <FiRotateCw />,
                        onClick: () => handleRestore(medicineId, name)
                    },
                ] as any[];
                        ...
                        <div className="col-12">
                            <div className="card">
                                <div className="card-body text-center">
                                    ...
                                    <button className="btn btn-primary" onClick={fetchDeletedMedicines}>
                    ...
                    <div className="col-12">
                        <div className="card">
                            <div className="card-header">
                                <h5 className="card-title mb-0">Restore Medicine</h5>
                            </div>
                            <div className="card-body">
                                {medicines.length === 0 ? (
                                    <div className="text-center py-5">
                                        <p className="text-muted">There are no deleted medicines to restore at this time.</p>
                                    </div>
                                ) : (
...
export default MedicineRestore;
