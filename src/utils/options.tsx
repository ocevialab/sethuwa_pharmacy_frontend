...
const propasalLeadOptions = [
    { value: 'web', label: 'Ocevia Lab - Website design and development', img: "/images/avatar/1.png" },
...
const employeeListStatusOptions = [
    { value: 'active', label: 'Active', color: '#17c666' },
    { value: 'inactive', label: 'Inactive', color: '#ffa21d' },
];

// Get employee role options based on pharmacy_role from localStorage
const getEmployeeRoleOptions = () => {
    try {
        const role = localStorage.getItem('pharmacy_role');
        if (role === 'OWNER') {
            return [
                { value: 'OWNER', label: 'OWNER', color: '#3454d1' },
                { value: 'ADMIN', label: 'ADMIN', color: '#41b2c4' },
                { value: 'CASHIER', label: 'CASHIER', color: '#17c666' },
                { value: 'DISPENSER', label: 'DISPENSER', color: '#ffa21d' },
                { value: 'ADMIN2', label: 'ADMIN2', color: '#6610f2' },
            ];
        }
        return [];
    } catch (error) {
